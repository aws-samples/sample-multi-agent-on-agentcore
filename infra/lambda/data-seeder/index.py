"""Data Seeder Lambda — populates DynamoDB tables with fixture data.

Runs as a CloudFormation Custom Resource on every deploy.
Writes per-user data to the user table (PK=username, SK=DOMAIN#TYPE#ID)
and global reference data to the global table (PK=CATEGORY#key, SK=DETAIL).
"""
...
# ============================================================================
# Per-user fixture data (PK=username, SK=DOMAIN#TYPE[#ID])
# ============================================================================
...
PERFORMANCE_REVIEWS = {
    "alice": {"cycle": "FY2026-H1", "status": "pending_submission", "self_rating": "", "goals_completed": "4/5", "feedback_received": "6", "next_deadline": "2026-03-31"},
    ...
    "charlie": {"cycle": "FY2026-H1", "status": "pending_submission", "self_rating": "", "goals_completed": "5/6", "feedback_received": "3", "next_deadline": "2026-03-31"},
}
...
BUDGETS = {
    "alice": {
        "department": "Human Resources", "fiscal_year": "FY2026",
        "total_budget": "$450,000", "spent_ytd": "$112,480", "remaining": "$337,520", "utilization": "25%",
    ...
    "bob": {
        "department": "Engineering", "fiscal_year": "FY2026",
        "total_budget": "$2,400,000", "spent_ytd": "$523,200", "remaining": "$1,876,800", "utilization": "22%",
        "categories": json.dumps({
    ...
    "charlie": {
        "department": "Operations", "fiscal_year": "FY2026",
        "total_budget": "$620,000", "spent_ytd": "$148,750", "remaining": "$471,250", "utilization": "24%",
        "categories": json.dumps({
...
MEETING_NOTES = {
    "alice": [
        {"notes_id": "MN-2026-HR-0142", "meeting": "HR Planning - Headcount Review", "date": "2026-02-10",
    ...
    "bob": [
        {"notes_id": "MN-2026-ENG-0158", "meeting": "Sprint 12 Retrospective", "date": "2026-02-07",
    ...
    "charlie": [
        {"notes_id": "MN-2026-OPS-0125", "meeting": "Q1 Business Review Prep", "date": "2026-02-12",
...
# ============================================================================
# Global reference data (PK=CATEGORY#key, SK=DETAIL)
# ============================================================================

COMPANY_POLICIES = {
    "pto_policy": {
        "title": "Paid Time Off Policy", "version": "v3.2 (Jan 2026)",
    ...
    "remote_work_policy": {
        "title": "Remote Work & Flexible Work Policy", "version": "v2.1 (Sep 2025)",
    ...
    "expense_policy": {
        "title": "Employee Expense Reimbursement Policy", "version": "v4.0 (Feb 2026)",
    ...
    "code_of_conduct": {
        "title": "Code of Conduct & Ethics Policy", "version": "v2.0 (Jan 2025)",
...
OFFICE_LOCATIONS = {
    "san_francisco": {
        "city": "San Francisco, CA", "address": "101 Market Street, Suite 1500, San Francisco, CA 94105",
        "floors": "15-17", "capacity": "350",
    ...
    "seattle": {
        "city": "Seattle, WA", "address": "1420 Fifth Avenue, Suite 800, Seattle, WA 98101",
        "floors": "8-9", "capacity": "200",
    ...
    "austin": {
        "city": "Austin, TX", "address": "300 W 6th Street, Suite 1800, Austin, TX 78701",
        "floors": "18", "capacity": "150",
...
def build_user_items():
    """Build all per-user DynamoDB items from fixture data."""
    items = []
    for username in ["alice", "bob", "charlie"]:
        # HR: Profile
        if username in USERS:
            items.append({"PK": username, "SK": "HR#PROFILE", **USERS[username]})

        # HR: PTO Balance
        if username in PTO_BALANCES:
            items.append({"PK": username, "SK": "HR#PTO_BALANCE", **PTO_BALANCES[username]})

        # HR: PTO Requests
        for req in PTO_REQUESTS.get(username, []):
            items.append({"PK": username, "SK": f"HR#PTO_REQUEST#{req['request_id']}", **req})

        # HR: Performance Review
        if username in PERFORMANCE_REVIEWS:
            items.append({"PK": username, "SK": "HR#PERFORMANCE_REVIEW", **PERFORMANCE_REVIEWS[username]})

        # HR: Open Positions
        for pos in OPEN_POSITIONS.get(username, []):
            items.append({"PK": username, "SK": f"HR#OPEN_POSITION#{pos['req_id']}", **pos})

        # HR: Onboarding Checklist
        for idx, task in enumerate(ONBOARDING_CHECKLISTS.get(username, [])):
            items.append({"PK": username, "SK": f"HR#ONBOARDING#{idx}", **task})

        # IT: Tickets
        for ticket in IT_TICKETS.get(username, []):
            items.append({"PK": username, "SK": f"IT#TICKET#{ticket['ticket_id']}", **ticket})

        # IT: Software Access
        for sw in SOFTWARE_ACCESS.get(username, []):
            items.append({"PK": username, "SK": f"IT#SOFTWARE#{sw['tool']}", **sw})

        # IT: Equipment
        for eq in EQUIPMENT_INVENTORY.get(username, []):
            items.append({"PK": username, "SK": f"IT#EQUIPMENT#{eq['serial']}", **eq})

        # Finance: Expenses
        for exp in EXPENSE_REPORTS.get(username, []):
            items.append({"PK": username, "SK": f"FIN#EXPENSE#{exp['expense_id']}", **exp})

        # Finance: Budget
        if username in BUDGETS:
            items.append({"PK": username, "SK": "FIN#BUDGET", **BUDGETS[username]})

        # Finance: Invoices
        for inv in INVOICES.get(username, []):
            items.append({"PK": username, "SK": f"FIN#INVOICE#{inv['invoice_id']}", **inv})

        # Productivity: Calendar
        for event in CALENDAR_EVENTS.get(username, []):
            items.append({"PK": username, "SK": f"PROD#CALENDAR#{event['event_id']}", **event})

        # Productivity: Documents
        for doc in DOCUMENTS.get(username, []):
            items.append({"PK": username, "SK": f"PROD#DOCUMENT#{doc['doc_id']}", **doc})

        # Productivity: Meeting Notes
        for note in MEETING_NOTES.get(username, []):
            items.append({"PK": username, "SK": f"PROD#MEETING_NOTES#{note['notes_id']}", **note})

    return items


def build_global_items():
    """Build all global DynamoDB items from fixture data."""
    items = []

    for key, policy in COMPANY_POLICIES.items():
        items.append({"PK": f"POLICY#{key}", "SK": "DETAIL", **policy})

    for key, office in OFFICE_LOCATIONS.items():
        items.append({"PK": f"OFFICE#{key}", "SK": "DETAIL", **office})

    for key, section in EMPLOYEE_HANDBOOK.items():
        items.append({"PK": f"HANDBOOK#{key}", "SK": "DETAIL", **section})
...
def batch_write(table, items):
    ...
    with table.batch_writer() as batch:
        for item in items:
            # Remove empty string values (DynamoDB doesn't allow empty strings in some contexts)
            cleaned = {k: v for k, v in item.items() if v != ""}
            batch.put_item(Item=cleaned)
    logger.info(f"Wrote {len(items)} items to {table.table_name}")


def handler(event, context):
    """CDK Provider onEvent handler. Return dict on success, raise on failure."""
    request_type = event.get('RequestType', 'Create')
    logger.info(f"Data seeder: RequestType={request_type}")
    ...
    if request_type in ('Create', 'Update'):
        ...
        global_table = dynamodb.Table(global_table_name)

        user_items = build_user_items()
        global_items = build_global_items()

        batch_write(user_table, user_items)
        batch_write(global_table, global_items)

        logger.info(f"Seeded {len(user_items)} user items, {len(global_items)} global items")

    return {'PhysicalResourceId': 'data-seed'}
